...
class Array:
    ...
    def check_ALU_available(self, dependent1, dependent2):
        ...
        for level in range(dep_level, len(self.levels)):
    ...
    def check_mult_available(self, dependent1, dependent2):
        ...
        dep_level = -1
        for level in self.levels:
            if level.register_in_memory(dependent1) or level.register_in_ALUs(dependent1) or level.register_in_memory(dependent2) or level.register_in_ALUs(dependent2):
                dep_level = self.levels.index(level)
        ...
        for level_i in range(dep_level+1, len(self.levels)):
            ...
            for i in range(len(level.mult_target)):
                if level.mult_target[i] == dependent1 or level.mult_target == dependent2:
                    dep_mul = i
                    dep_level = level_i
                    break
    ...
    def set_mult(self, words):
        (mult, level) = self.check_mult_available(words[1])
        if mult == -1:
            return False
    ...
    def check_memory_available(self, dependent1, dependent2):
        ...
        for level_i in range(dep_level+1, len(self.levels)):
            ...
            for i in range(len(level.memory_target)):
                if level.memory_target[i] == dependent1 or level.memory_target == dependent2:
                    dep_mem = i
                    dep_level = level_i
    ...
    def set_memory(self, words):
        (mem, level) = self.check_memory_available(words[1], words[2])
        if mem == -1:
            return False
